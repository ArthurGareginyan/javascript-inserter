...
function spacexchimp_p016_textdomain() {
    load_plugin_textdomain( SPACEXCHIMP_P016_TEXT, false, SPACEXCHIMP_P016_DIR . '/languages/' );
}
add_action( 'init', 'spacexchimp_p016_textdomain' );
...
function spacexchimp_p016_settings_link( $links ) {
    $page = '<a href="' . admin_url( 'options-general.php?page=' . SPACEXCHIMP_P016_SLUG ) . '">' . __( 'Settings', SPACEXCHIMP_P016_TEXT ) . '</a>';
...
add_filter( 'plugin_action_links_' . SPACEXCHIMP_P016_BASE, 'spacexchimp_p016_upgrade_link' );
...
function spacexchimp_p016_plugin_row_meta( $links, $file ) {
    ...
    if ( strpos( $file, SPACEXCHIMP_P016_SLUG . '.php' ) !== false ) {

        $new_links = array(
                           'donate' => '<a href="https://www.spacexchimp.com/donate.html" target="_blank"><span class="dashicons dashicons-heart"></span> ' . __( 'Donate', SPACEXCHIMP_P016_TEXT ) . '</a>',
                           'upgrage' => '<a href="https://www.spacexchimp.com/plugins/javascript-inserter-pro.html" target="_blank"><span class="dashicons dashicons-star-filled"></span> ' . __( 'Upgrade to PRO', SPACEXCHIMP_P016_TEXT ) . '</a>'
                           );
...
add_filter( 'plugin_row_meta', 'spacexchimp_p016_plugin_row_meta', 10, 2 );
...
function spacexchimp_p016_register_submenu_page() {
    ...
    $page_title  = SPACEXCHIMP_P016_NAME;
    $menu_title  = __( 'JS Inserter', SPACEXCHIMP_P016_TEXT );
    $capability  = 'manage_options';
    $menu_slug   = SPACEXCHIMP_P016_SLUG;
    $function    = 'spacexchimp_p016_render_submenu_page';

    add_options_page( $page_title, $menu_title, $capability, $menu_slug, $function );
}
add_action( 'admin_menu', 'spacexchimp_p016_register_submenu_page' );

/**
 * Register settings
 */
function spacexchimp_p016_register_settings() {
    register_setting( SPACEXCHIMP_P016_SETTINGS . '_settings_group', SPACEXCHIMP_P016_SETTINGS . '_settings' );
    register_setting( SPACEXCHIMP_P016_SETTINGS . '_settings_group_si', SPACEXCHIMP_P016_SETTINGS . '_service_info' );
}
add_action( 'admin_init', 'spacexchimp_p016_register_settings' );
...
function spacexchimp_p016_admin_footer_text() {
    ...
    $settings_page = 'settings_page_' . SPACEXCHIMP_P016_SLUG;
    ...
    add_filter( 'admin_footer_text', 'spacexchimp_p016_new_admin_footer_text', 11 );
}
add_action( 'current_screen', 'spacexchimp_p016_admin_footer_text' );
...
function spacexchimp_p016_activation() {
    ...
    if ( $activation_date == '' ) {
        ...
        update_option( SPACEXCHIMP_P016_SETTINGS . '_service_info', $info );
    }
}
register_activation_hook( SPACEXCHIMP_P016_FILE, 'spacexchimp_p016_activation' );

/**
 * Delete options on uninstall
 */
function spacexchimp_p016_uninstall() {
    delete_option( SPACEXCHIMP_P016_SETTINGS . '_settings' );
}
register_uninstall_hook( SPACEXCHIMP_P016_FILE, 'spacexchimp_p016_uninstall' );
